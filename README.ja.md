...
> **修正フォーク: upstream 1.8.0 / ビルド 180.1。** GPT の修正、混合入力、音声録音を統合しました。[レビューとビルド手順](FORK.md)をご覧ください。このフォークでは録音は初期設定で **オン**、テキスト保存は **オフ** です。以下のダウンロードは修正を含まない upstream リリースです。
