...
- **선택형 녹음, 기본 활성화:** 캡처 중 마이크 또는 Mac 오디오를 기록 파일과 같은 폴더에 압축된 `.m4a`로 함께 저장합니다. 메인 화면의 **녹음 파일 저장** 체크박스로 끌 수 있습니다. 저장된 `.m4a` 파일은 Mac에 남지만, API 기반 모드는 캡처한 오디오를 선택한 제공자에게 실시간 전송합니다.
...
- **번역문 띄어쓰기 수정(GPT 실시간 번역):** 제공자 턴을 구간 경계에서 결합해 `배송되고거기서`처럼 붙어 나오던 문제를 해결했습니다. Gemini Live는 기존 방식을 유지합니다.
